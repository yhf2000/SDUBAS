...
async def file_upload_valid(request: Request, file: file_interface, user_agent: str = Header(None),
    ...
    if id is None:  # 没有该file
        ...
        return {'message': '文件不存在', 'data': {'token': new_token}}
    else:  # 有该file
...
@user_standard_response
async def file_upload(request: Request, token: str = Header(None), file: UploadFile = File(...)):
    old_session = json.loads(session_db.get(token))  # 有效session中没有
    ...
    add_operation(service_type=7, service_id=id, func='上传文件', parameters='用户上传了一个文件',
                  oper_user_id=file_information.user_id)  # 添加一个文件上传的操作
    data = dict()
...
async def file_download(id: int, request: Request, user_agent: str = Header(None)):
    ...
    user_session = json.dumps(new_session.model_dump())
    session_db.set(new_token, user_session, ex=21600)  # 缓存有效session(时效6h)
    return {'message': '请前往下载', 'data': {'token': new_token}}


@files_router.get("/download/files")  # 下载文件
async def file_download_files(request: Request, token: str = Header(None)):
    old_session = json.loads(session_db.get(token))  # 有效session中没有
    if old_session is None:
        raise HTTPException(
            status_code=401,
    ...
    add_operation(service_type=7, service_id=old_session['file_id'], func='下载文件', parameters='用户下载了一个文件',
                  oper_user_id=old_session['user_id'])
...
async def file_preview(pageNow: int, pageSize: int,  session=Depends(auth_login)):
    ...
    return {'message': '文件列表如下', 'pageIndex': pageNow, "pageSize": pageSize, "totalNum": len(all_file),
            "rows": file_data}
