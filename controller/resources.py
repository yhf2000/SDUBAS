...
async def save_api(apiSchema: financial_Basemodel.ResourceAdd, user=Depends(auth_permission)):
    ...
    return db.save_resource(obj_in=apiSchema, user_id=user)
...
async def get_resource_by_user(pageNow: int = Query(description="页码", gt=0),
    ...
    tn, res = db.get_resource_by_user(user=user, pg=Page, user_id=user)
...
async def Update_resource_by_count(resource_id: int, apiSchema: financial_Basemodel.resource_count_update,
    ...
    db = ResourceModel()  # 判断后直接添加
    Resource = db.check_by_id(Id=resource_id, user_id=user)
    if Resource is not None:
        return db.count_Update(Id=resource_id, count=apiSchema.count, user_id=user)
    else:
...
async def Update_resource_by_count(resource_id: int,
                                   user=Depends(auth_permission)):
    db = ResourceModel()
    return db.check_by_id(Id=resource_id, user_id=user)
...
async def delete(resource_id: int, user=Depends(auth_permission)):
    db = ResourceModel()
    check_result = db.check_by_id(Id=resource_id, user_id=user)  # 判断有无资源，是否被删除
    if check_result is not None:  # 有，删除
        return_id = db.delete(Id=resource_id, user_id=user)
        return return_id
    else:  # 无，项目找不到
        raise HTTPException(status_code=404, detail="Item not found")
...
async def save_financial(apiSchema: financial_Basemodel.FinancialAdd, user=Depends(auth_permission)):
    db = FinancialModel()
    return db.save_financial(obj_in=apiSchema, user_id=user)
...
async def save_financial(financial_id: int, apiSchema: financial_Basemodel.AmountAdd, user=Depends(auth_permission)):
    ...
    num = FinancialModel_db.check_by_id(Id=financial_id, user_id=user)  # 所有信息，处理总额之外所有信息
    if num is None:
        raise HTTPException(status_code=404, detail="Item not found")
    else:
        apiSchema.finance_id = financial_id
        return db.save_amount(obj_in=apiSchema, user_id=user)
...
async def query_total(financial_id: int, user=Depends(auth_permission)):
    ...
    if num is None:
        ...
        summ = BillModel_db.query_total(Id=financial_id, user_id=user)  # 获取可用流水，总额
...
async def query_page(financial_id: int, pageNow: int = Query(description="页码", gt=0),
    ...
    FinancialModel_db = FinancialModel()
    BillModel_db = BillModel()
    num = FinancialModel_db.check_by_id(Id=financial_id, user_id=user)  # 检查资金项目有无
    if num is None:
        raise HTTPException(status_code=404, detail="Item not found")
    else:
        tn, res = BillModel_db.query_amount(ID=financial_id, pg=Page, user_id=user)  # 返回总额，分页数据
...
async def delete_financial(financial_id: int, user=Depends(auth_permission)):
    FinancialModel_db = FinancialModel()
    BillModel_db = BillModel()
    check_result = FinancialModel_db.check_by_id(Id=financial_id, user_id=user)
    if check_result is not None:
        return_id = FinancialModel_db.delete(Id=financial_id, user_id=user)
        BillModel_db.delete_by_financial(Id=financial_id, user_id=user)  # 同步删除所有资金流水
        # 此处应该删除权限
        return return_id
    else:
        raise HTTPException(status_code=404, detail="Item not found")
...
@standard_response
async def delete_account(financial_id: int, account_id: int, user=Depends(auth_permission)):
    BillModel_db = BillModel()
    check_result = BillModel_db.check_by_id(Id=account_id, user_id=user)
    if check_result is not None:
        return_id = BillModel_db.delete_by_id(Id=account_id, user_id=user, financial_id=financial_id)
        return return_id
    else:
        raise HTTPException(status_code=404, detail="Item not found")
...
async def delete_financial(financial_id: int, apiSchme: financial_Basemodel.FinancialUpdate,
    ...
    check_result = FinancialModel_db.check_by_id(Id=financial_id, user_id=user)
    if check_result is not None:
        returnId = FinancialModel_db.note_Update(Id=financial_id, note=apiSchme.note, user_id=user)
        return returnId
    else:
        raise HTTPException(status_code=404, detail="Item not found")
...
async def get_financial_by_user(pageNow: int = Query(description="页码", gt=0),
    ...
    db = FinancialModel()
    # 调用函数
    tn, result = db.get_financial_by_user(user=user, pg=Page, user_id=user)
