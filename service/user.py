...
from model.user import User, User_info, Session, School, College, Major, Class
from type.user import register_interface, user_info_interface, session_interface, \
    school_interface, college_interface, class_interface, major_interface


class UserModel(dbSession):
    def add_user(self, obj: register_interface):  # 用户注册(在user表中添加一个用户)
...
class UserinfoModel(dbSession):
    def add_userinfo(self, obj: user_info_interface, user_id):  # 在user_info表中添加一条信息
        obj.user_id = user_id
        obj_dict = jsonable_encoder(obj)
    ...
    def get_class_by_name(self, obj: class_interface):  # 根据班级名和学院id和学校id查询班级
        with self.get_db() as session:
            clas = session.query(Class).outerjoin(College, Class.college_id == College.id).outerjoin(School,
                                                                                                     Class.has_delete == 0,                                                                                     College.school_id == School.id).filter(
                College.id == obj.college_id,
                Class.name == obj.name,
                School.id == obj.school_id
